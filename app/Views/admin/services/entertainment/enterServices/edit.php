        ...
        <form action="<?php echo _HOST_PATH_ ?>/admin/EnterServices/postEditEnterServices/id = <?php echo $arrData['enterservice_id'] ?>" method="post" class="w-50">
            ...
            <!-- <div class="mb-3 d-flex">
                ...
                <div class="w-50">
                    ...
                    <input type="text" name="height_limit" id="" class="form-control text-left" placeholder="Nhập quy định (nếu có)" value="<?php echo ($arrData['height_limit']) ? $arrData['height_limit'] : '' ?>">
                </div>
            </div> -->

            <div class="mb-3 d-flex">
                <div class="w-50 me-3">
                    <label for="">Quy định độ tuổi</label>
                    <input required type="number" name="age_limit" id="" class="form-control" placeholder="Nhập độ tuổi tối thiểu" value="<?php if (isset($old['age_limit'])) {
                                                                                                                                                echo $old['age_limit'];
                                                                                                                                            } else {
                                                                                                                                                echo $arrData['age_limit'];
                                                                                                                                            } ?>">
                </div>
                <div class="w-50">
                    <label for="">Quy định chiều cao(mét)</label>
                    <select name="height_limit" id="" class="form-control">
                        <option <?php if (isset($old)) {
                                    if ($old['height_limit'] == 0) {
                                        echo 'selected';
                                    }
                                } else if ($arrData['height_limit'] == 0) {
                                    echo 'selected';
                                } ?> value="0">Không giới hạn</option>
                        <option <?php if (isset($old)) {
                                    if ($old['height_limit'] == 1) {
                                        echo 'selected';
                                    }
                                } else if ($arrData['height_limit'] == 1) {
                                    echo 'selected';
                                } ?> value="1">Từ 1m - 1,3m</option>
                        <option <?php if (isset($old)) {
                                    if ($old['height_limit'] == 1.3) {
                                        echo 'selected';
                                    }
                                } else if ($arrData['height_limit'] == 1.3) {
                                    echo 'selected';
                                } ?> value="1.3">Trên 1,3m</option>
                    </select>
                </div>
            </div>
            <div class="mb-3">
