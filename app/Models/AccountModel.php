...
class AccountModel extends Model
{
    ...
    public function checkExistAccount($un) {
        $sql = "SELECT count(*) FROM account WHERE username = '$un'";
        $cnt = $this->countLine($sql);
        return $cnt == 0;
    }
    ...
    public function getUsernameByUserId($id) {
        $sql = "SELECT username from account WHERE user_id = $id";
        $username = $this->Select($sql);
        return $username;
    }
